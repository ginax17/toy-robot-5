...
"""
TODO: You can either work from this skeleton, 
or you can build on your solution for Toy Robot 3 exercise.
"""
import sys
# import world
import turtle
...
if len(sys.argv) == 2 and sys.argv[1] == 'text' :
    ...
    obstacle_choice = "obstacles"

elif len(sys.argv) == 3 and sys.argv[2] == 'simple' :
    from world.text import world as world
    obstacle_choice = "empty maze"

elif len(sys.argv) == 2 and sys.argv[1] == 'turtle':
    from world.turtle import world as world
    obstacle_choice = "obstacles"

elif len(sys.argv) == 3 and (sys.argv[1] == 'turtle' \
 or sys.argv[1] == 'text') and len(sys.argv[2]) > 0:

    if sys.argv[1] == 'turtle':
        from world.turtle import world as world
    elif sys.argv[1] == 'text':
        from world.text import world as world
    obstacle_choice = sys.argv[2]
    
else: 
    from world.text import world as world
    obstacle_choice = "obstacles"

# new_turtle = turtle.getscreen()
# turtle_reference.right(90)
# turtle_reference.forward(30)
# turtle_reference.left(90)
# turtle_reference.back(100)
# turtle_reference.circle(80)
...
def split_command_input(command):
    ...
    to get the actual command, as well as
...
def valid_command(command):
    ...
    """
    mazerun_arg = ["top", "left", "right", "bottom"]
    
    (command_name, arg1) = split_command_input(command)
    if command_name == "mazerun":
        if arg1 in mazerun_arg:
            return True
...
def do_forward(robot_name, steps):
    ...
    """
    move = world.update_position(steps, current_direction_index)
    if move[0] == True:
        #world.turtle_reference.forward(steps)
...
def do_back(robot_name, steps):
    ...
    if world.update_position(-steps,current_direction_index):
        #world.turtle_reference.back(steps)
...
def do_right_turn(robot_name):
    ...
    #world.turtle_reference.right(90)
...
def do_left_turn(robot_name):
    ...
    if current_direction_index < 0:
        ...
        #world.turtle_reference.left(90)
...
def call_command(command_name, command_arg, robot_name):
    ...
    if command_name == "mazerun":
        return world.mazerunner_main(command_arg)
    return False, None
...
def robot_start():
    ...
    obstacle = import_helper.dynamic_import(f'maze.{obstacle_choice}')
    print(robot_name+": loaded",obstacle_choice)
    obstacle.obstacles()
    obstaclez = obstacle.obstacles_list
   
    if len(obstaclez)> 0:
        world.print_obstacles(obstaclez)
